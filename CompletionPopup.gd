...
func secs_to_str(completion_time):
	...
	if completion_time > 60:
		message.append('%d minutes' % int(completion_time / 60))
...
func journeys_end(completion_time, fastest_time):
	popup()
	var completion_time_str = secs_to_str(int(completion_time))
	...
	if completion_time == fastest_time:
		...
		$AdiosText.bbcode_text = text.replace('YYY', secs_to_str(int(fastest_time)))
